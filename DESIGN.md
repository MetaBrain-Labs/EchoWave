...
当前版本规定文字颜色、字体、排版层级、布局间距和圆角，不规定字重、背景、边框或其他组件视觉属性。未在本文档中定义的视觉规则不得从示例中自行推导。
...
EchoWave 使用 Source Han Sans CN 和 KaiTi 两套字体。

| 字体 | 使用范围 |
| --- | --- |
| Source Han Sans CN | 页面标题、导航、按钮、筛选、排序、元数据及其他常规界面文字 |
| KaiTi | 转写正文，以及情绪名称、情绪标签、情绪描述和情绪摘要 |
...
- 具体字体文件和字重须在合法字体资源或新增设计要求确定后另行规定，不得由实现者自行假定。字号与行高必须遵循本文档的排版层级。
...
当前 `spacing`、`radii` 和 `typeScale` 令牌尚未完全符合本文档新增的间距、圆角和排版规则。本文档描述的是后续实现必须遵守的目标规范，不表示这些令牌和现有页面已经完成对齐。

当前仓库尚未包含 Source Han Sans CN 或 KaiTi 字体资源，也未实现字体加载流程。本文档描述的是后续实现必须遵守的目标规范，不表示字体已经接入。

## 8. 变更边界

本文档不授权自行扩展字体、文字颜色、排版层级、间距、圆角或其他视觉规则。新增文字语义、状态色文字、字体用途、排版层级或例外值前，应先更新本文档并完成设计确认。
