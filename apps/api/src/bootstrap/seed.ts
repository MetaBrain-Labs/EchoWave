...
async function seedPublishedAnalysis(client: PoolClient, value: SeedAnalysisIds) {
  ...
  const segmentRows = [
    [value.segments[0], value.sceneOne, 1, 'host', '主持人', '专注', 0, 28000,
      '今天想和你聊聊最近使用团队音频整理工具的体验。先从日常工作开始，你通常会在什么场景下记录和回听访谈？'],
    [value.segments[1], value.sceneOne, 2, 'self', '我', '平静', 29000, 71000,
      '最常见的是用户访谈和每周复盘。我会先完整录音，结束后再回听并整理重点，但在很长的录音里寻找关键内容会花不少时间。'],
    [value.segments[2], value.sceneTwo, 1, 'host', '主持人', '好奇', 84000, 112000,
      '如果工具可以自动完成一部分整理工作，你最希望它优先解决什么问题？'],
    [value.segments[3], value.sceneTwo, 2, 'self', '我', '期待', 113000, 168000,
  ...
  const tagRows = [
    [value.tags[0], value.segments[0], '高频访谈记录场景',
      '受访者的工作流程依赖持续记录和会后回顾，音频整理是研究过程中的固定环节。',
      ['主要场景包括用户访谈、项目复盘和跨团队评审。', '使用者希望保留原始语境，同时快速定位能够支持结论的片段。']],
    [value.tags[1], value.segments[2], '优先需求：快速定位证据',
      '用户最重视定位效率，希望分析结果能够回到原始转写和准确时间点。',
      ['总结需要与具体说话人、时间点和原始表达关联。', '标签应该帮助筛选，不应替代可核对的转写内容。']],
  ] as const;
...
async function seed() {
  ...
  try {
    ...
    const sources = [
      [ids.sources[0], '团队录音空间', '汇集团队访谈、周会与客户沟通录音。', 'manual_upload', 'local', 'HTTPS API / team-audio'],
      [ids.sources[1], '用户研究云盘', '同步研究项目中的访谈音频与观察记录。', 'cloud_drive', 'cloud', 'Cloud Drive / research'],
      [ids.sources[2], '客户沟通归档', '接入客户成功团队的沟通录音。', 's3', 'cloud', 'S3 / customer-calls'],
      [ids.sources[3], '市场调研资料', '整理市场活动与竞品调研录音。', 'local_folder', 'local', 'Local Folder / market'],
    ] as const;
    ...
    const sourceLinks = [
      [ids.groups[0], ids.sources[0]], [ids.groups[0], ids.sources[1]],
      [ids.groups[1], ids.sources[0]], [ids.groups[1], ids.sources[2]],
      [ids.groups[2], ids.sources[0]], [ids.groups[2], ids.sources[3]],
    ];
    ...
    for (const [index, audio] of audioRows.entries()) {
      await client.query(
        ...
        [
          ids.audio[index], tenantId, ids.sources[0], audio[4], audio[0], `${audio[0]}.mp3`,
          audio[1], `demo-audio-${index + 1}`, audio[2], audio[3],
    ...
    await seedPublishedAnalysis(client, {
      analysis: ids.analyses[0], audio: ids.audio[0],
      sceneOne: '60000000-0000-4000-8000-000000000001', sceneTwo: '60000000-0000-4000-8000-000000000002',
      segments: ['70000000-0000-4000-8000-000000000001', '70000000-0000-4000-8000-000000000002', '70000000-0000-4000-8000-000000000003', '70000000-0000-4000-8000-000000000004'],
      ...
      summaries: ['80000000-0000-4000-8000-000000000001', '80000000-0000-4000-8000-000000000002', '80000000-0000-4000-8000-000000000003', '80000000-0000-4000-8000-000000000004', '80000000-0000-4000-8000-000000000005'],
      publishedAt: '2026-08-15T10:51:24.000Z',
    });
    await seedPublishedAnalysis(client, {
      analysis: ids.analyses[1], audio: ids.audio[1],
      sceneOne: '60000000-0000-4000-8000-000000000011', sceneTwo: '60000000-0000-4000-8000-000000000012',
      segments: ['70000000-0000-4000-8000-000000000011', '70000000-0000-4000-8000-000000000012', '70000000-0000-4000-8000-000000000013', '70000000-0000-4000-8000-000000000014'],
      ...
      summaries: ['80000000-0000-4000-8000-000000000011', '80000000-0000-4000-8000-000000000012', '80000000-0000-4000-8000-000000000013', '80000000-0000-4000-8000-000000000014', '80000000-0000-4000-8000-000000000015'],
