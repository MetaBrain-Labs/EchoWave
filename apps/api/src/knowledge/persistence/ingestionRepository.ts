...
export class IngestionRepository {
  ...
  async claimIngestionJob(): Promise<ClaimedIngestionJob | undefined> {
    ...
    return {
      id: row.id, tenantId: row.tenant_id, knowledgeBaseId: row.knowledge_base_id,
      documentId: row.document_id, revisionId: row.revision_id, stagedPath: row.staged_path,
      title: row.title, format: row.format, sizeBytes: Number(row.size_bytes), attempts: row.attempts,
    };
  }
