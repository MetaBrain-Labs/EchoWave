...
import type { OpenRouterEmbeddings } from './openRouterEmbeddings.ts';
import type { DeepSeekQueryAgent } from './queryAgent.ts';
import { RagRepositoryError, type RagRepository, type RetrievalChunk } from './repository.ts';
...
type KnowledgeAnswerRepository = Pick<
  RagRepository,
...
class DefaultKnowledgeAnswerModule implements KnowledgeAnswerModule {
  ...
  async answer(command: KnowledgeAnswerCommand): Promise<RagQueryResponse> {
    ...
    const conversation = await this.options.repository.getOrCreateConversation(
    ...
    try {
      const generated = await this.options.agent.generate({
        ...
        searchKnowledge: async (query) => {
          ...
          const chunks = await this.options.repository.search(
            command.knowledgeBaseId,
            embedded.vectors[0] ?? [],
          );
  ...
  private async cleanupExpiredConversations(): Promise<void> {
    for (const conversation of await this.options.repository.listExpiredConversations()) {
      try {
        // checkpoint 必须先删除；随后数据库外键才能安全清理 conversation 与关联 run。
        await this.options.checkpointer.deleteThread(conversation.threadId);
        await this.options.repository.deleteExpiredConversation(conversation.id);
      } catch (error) {
