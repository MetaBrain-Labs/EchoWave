...
/** 渲染可点击并可与滑动分页同步的同级标签。 */
export function PageTabs<Tab extends string>({ activeTab, onChange, tabs }: {
  activeTab: Tab;
      ...
      {tabs.map((tab) => {
        ...
        return (
          <Pressable key={tab.key} accessibilityRole="tab" accessibilityState={{ selected }}
            onPress={() => onChange(tab.key)} style={({ pressed }) => [styles.tab, pressed && styles.pressed]}>
...
const styles = StyleSheet.create({
  ...
  tabs: { flexDirection: 'row', gap: spacing.lg, minHeight: 32, paddingHorizontal: spacing.md },
  tab: { justifyContent: 'flex-end' },
  tabText: {
    ...typography.heading5, color: textColors.secondary, fontFamily: fontFamilies.sansBold,
    fontWeight: 'bold', paddingBottom: spacing.xs,
  },
