...
- `qwen3.5-omni-flash` is the default text model for knowledge answers, business role, speaker review, and business analysis, and also handles per-segment acoustic emotion through the Beijing OpenAI-compatible endpoint and signed OSS URLs. Emotion results must cover each target and include the fixed emotion enum, confidence, and acoustic cues.
- Text capabilities bind a provider connection plus a model chosen from the provider's model catalog. DashScope candidates come from `GET /api/v1/models` filtered by capability responsibility (text generation, plus audio input for emotion); DeepSeek keeps a static `deepseek-v4-flash` fallback for cost-sensitive deployments. The server re-checks the model against the catalog when a binding is saved, so a model outside its capability can never be published.
- Thinking parameters are provider-specific: DeepSeek expects `thinking: { type }` and Model Studio's compatible mode expects `enable_thinking`. The shared chat-model factory emits only the field the bound provider understands.
- Role recognition uses non-thinking JSON output with the core roles Sales/Customer/Other/Unknown (销售/客户/其他/未知 in Chinese) plus up to 16 custom roles configured on the data source.
- Retrieval uses cosine HNSW with `ef_search=100` to recall at most 20 candidates. When the tenant switch and frozen `knowledge_rerank` binding are usable, DashScope `qwen3.7-text-rerank` reorders that same candidate set; an eight-second timeout, invalid response, rate limit, or missing configuration falls back to vector order without losing the query. Deduplication and per-document quotas then select at most 5 chunks within the caller's character budget.
- Each RAG run and business-analysis job freezes whether reranking is enabled and the binding revision it may use. Completed retrieval audit records the applied/disabled/fallback status, model, candidate and selected counts, promoted evidence count, token usage, duration, and safe fallback reason; it never stores credentials, candidate bodies, or relevance scores.
