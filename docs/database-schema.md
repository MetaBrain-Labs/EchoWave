...
音频二进制、第三方连接凭据和页面 UI 偏好不进入这些业务表。数据库只保存权威业务事实；数量、总时长、最近上传时间和关联分组数量由查询聚合生成。
...
分组与数据源的多对多关联表。分组关联数据源后，该数据源下的所有未删除音频对分组可见，不需要逐条复制音频关联。
...
表中不保存音频二进制，只保存对象定位键和元数据；`deleted_at` 用于软删除。
