...
问答检索工具在现有模型调用中选择最多三个类别；移动端用户可以显式多选任意数量的类别且不设上限，两类选择共享同一套服务端校验。业务分析在已有规划调用中为每条查询选择类别。服务端在当前库或关联库白名单内再次校验，并通过 SQL 过滤生效类别，保留租户、文档状态、active revision、embedding 模型与现有上下文预算。普通事实任务排除已分类的测试样例。

从分组顶栏进入问答时，客户端默认勾选该知识库全部启用类别（停用类别不进入默认筛选），用户可自行增删；类别目录加载失败时退回自动路由，不阻塞提问。
