...
- 多轮上下文只保留最近六轮的“问题 + 最终回答”，历史轮次的 `search_knowledge` 工具消息与工具调用消息会被移除：历史段落不在本轮引用白名单内，留在上下文会让模型复述并引用上一个知识库的内容。
...
问答页与历史面板把正文中的 `[n]` 渲染为可点击标记：点击后展开折叠区、把滚动容器定位到对应引用卡片并临时高亮该卡片，偏移由卡片实测高度累加得出，不依赖固定高度假设。点击卡片仍然只打开引用快照弹层，不触发原文跳转；只有弹层里的“查看当前原文”才进入原文路由。
...
`040_knowledge_categories.sql` 增加租户类别目录、知识库默认类别、revision 人工覆盖、工作表覆盖、建议和确认历史。生效顺序为工作表覆盖、文档覆盖、知识库默认类别。未确认建议不参与生效分类；收集目录只表示来源，案例检索投影继承目标知识库类别。

上传分类在入库 Graph 的 `classify` 节点复用 `knowledge_chat`，一次请求公平采样所有工作表，总正文不超过 12000 字符。模型建议保存后，入库重试不再次调用分类模型；分类失败不阻断向量发布。已有文档通过详情面板按需获取建议；改名复制原 revision 分类，不把文件替换误当成原分类沿用。

问答检索工具在现有模型调用中选择最多三个类别；移动端用户可以显式多选任意数量的类别且不设上限，两类选择共享同一套服务端校验。检索范围为知识库集合：路由知识库必选，请求可携带同租户的其他知识库，服务端逐个校验存在性与租户归属，任一不可用即整次拒绝，避免静默缩小范围。单库沿用原有召回与上下文预算，多库改用跨库检索的全局配额；类别目录为各库目录的并集，因此按知识库分组展示。业务分析在已有规划调用中为每条查询选择类别。服务端在当前库或关联库白名单内再次校验，并通过 SQL 过滤生效类别，保留租户、文档状态、active revision、embedding 模型与现有上下文预算。普通事实任务排除已分类的测试样例。

从分组顶栏进入问答时，客户端默认勾选该分组全部关联知识库的启用类别（停用类别不进入默认筛选），用户可增删类别，也可在类别面板里按知识库取消参与检索；类别目录整体加载失败时退回自动路由，不阻塞提问。**每次进入即一段新聊天**：入口带上新的 `session` 参数，页面重挂载、记忆清空；该段聊天一旦发出第一个问题，检索类别与知识库范围即冻结并提示“退出后重新进入可重新选择”，同一段聊天内的后续提问共享多轮记忆。会话与运行记录通过 `knowledge_base_ids` 保存本次实际检索范围，会话仍锚定主知识库以便历史列表与过期清理。

自动路由零命中或证据不足可共享一次扩大检索，复用查询向量；显式筛选不扩大。问答仍限制四次实际检索，业务分析最多三次主动检索加两次补充检索，扩大查询也计入共用的五次 SQL 预算。业务分析预算与扩大标记持久化，恢复和并行分支不重置；知识内容版本也覆盖分类和目录变化，防止旧范围结果发布。

`/api/knowledge-categories` 提供目录创建与版本更新，不提供硬删除；`/api/knowledge-bases/:knowledgeBaseId/categories` 返回实际可检索类别。文档 `classification` 子资源读取及确认覆盖，`classification/suggest` 按需获取建议。问答请求可选 `categoryIds`。类别调整仅更新元数据，正文、embedding、内容哈希和历史引用快照保持不变。

分类索引配合 HNSW 迭代扫描；缩小结果范围不代表扫描成本同比下降，需要隔离数据集及执行计划验证。
