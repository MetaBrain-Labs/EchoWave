...
[English](./audio-analysis-automation.md) | **简体中文**

“新建”页可以把 1–20 个新文件或已有音频放入同一批次。批次固定一个数据源和一个分组；上传和音频校验立即执行，`scheduledFor` 只控制 ASR、情绪、角色与业务分析何时可领取。

持久化的“默认分析方式”统一影响手机录音和新建页文件导入，并且可以就地修改：手机录音页的“默认分析方式”选择，以及“新建”页“更多设置”内的同名选择都写入同一偏好。默认全流程。仅转写冻结 `confirmation: manual`，关闭情绪、角色和业务分析；ASR 成功即完成批次，不自动确认正文，后续由用户确认并选择单项分析。设置修改仅影响后续提交；“更多 → 通用设置”修改的是同一项设置。

分析详情页的“分析任务”分页在角色识别区块展示该音频所属数据源的角色词典：四个核心角色固定可用，自定义角色可在该页直接添加或移除。“新建”页的一键分析在选中数据源后同样展示该词典，切换数据源会随之切换。自定义角色保存在数据源上，启动角色识别时随任务输入快照固化，因此改动只对之后启动的任务生效；已发布结果不会被追溯修改，需要重新运行角色识别才能应用新角色。无数据源或数据源不可读时不展示该区块。

## 模式边界

三种模式都支持“上传完成后关闭 App，服务端后台继续处理”。这里的区别不是能不能异步，而是原音频保存在哪里，以及服务器异常、重启、迁移之后能恢复到什么程度。只有音频上传完成并成功创建/提交服务端任务后，App 生命周期才与后台任务生命周期解耦；上传尚未完成时直接杀掉 App，不能理解为服务端已经接管。

- 轻量本地 = 异步处理 + 临时本地存储 = 成本最低 / 恢复能力最低。支持立即批量，不接受未来时间；音频依赖阶段硬阻塞时源文件保留 24 小时，过期后必须重新选择 SHA-256 和字节数相同的文件。
- 混合模式 = 异步处理 + 持久本地存储 + OSS 中转 = 默认方案 / 成本和可靠性平衡。支持立即、批量、定时和恢复，但要求持久化 `AUDIO_STORAGE_DIR` 且当前保持单 API 实例。
- 对象存储 = 异步处理 + OSS 持久存储 = 云部署 / 大规模 / 恢复能力最好。支持立即、批量、定时和恢复，上传使用预签名 PUT。
