...
三种模式都需要 PostgreSQL、FFmpeg/VAD 和 DashScope 音频能力；文本类能力（知识问答、角色识别、说话人复核、业务分析）默认也由 DashScope 连接上的通义千问模型承担，因此单个百炼 API Key 即可跑通全部能力。只有把文本类能力改绑到 DeepSeek 连接（成本敏感时缓存命中价更低）才需要该连接。混合与对象存储模式必须先完成相应 OSS 能力绑定；轻量本地可以在没有 OSS 的情况下工作。
